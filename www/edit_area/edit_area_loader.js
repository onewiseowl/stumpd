...
EditAreaLoader.prototype ={
	...
	//	alert(settings["plugins"].length+": "+ settings["plugins"].join(","));
		t.get_template();
		t.load_script("/edit_area/" + "langs/"+ s["language"] + ".js");
		
		if(s["syntax"].length>0){
			s["syntax"]=s["syntax"].toLowerCase();
			t.load_script("/edit_area/reg_syntax/"+ s["syntax"] + ".js");
		}
		//alert(this.template);
	...
	start : function(id){
		...
		if(!t.iframe_script){
			...
			for(i=0; i<t.sub_scripts_to_load.length; i++)
				t.iframe_script+='<script language="javascript" type="text/javascript" src="/edit_area/' + t.sub_scripts_to_load[i] +'.js"></script>';
		}
		...
		for(i=0; i<area["settings"]["plugins"].length; i++){
			...
			if(!t.all_plugins_loaded)
				t.iframe_script+='<script language="javascript" type="text/javascript" src="'+ '/edit_area/plugins/' + area["settings"]["plugins"][i] + '/' + area["settings"]["plugins"][i] +'.js"></script>';
			t.iframe_script+='<script language="javascript" type="text/javascript" src="' '/edit_area/plugins/' + area["settings"]["plugins"][i] + '/langs/' + area["settings"]["language"] +'.js"></script>';
		}
